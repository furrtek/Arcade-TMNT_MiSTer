...
int main(int argc, char *argv[]) {
	...
	if (argc != 3) {
		puts("usage: rom2verilog [byte/word/dword/half] input.bin\n");
		return 1;
	}
    ...
    *strrchr(file_stem, '.') = 0;		// Doesn't seem very safe
    ...
    } else if (!strcmp(argv[1], "dword")) {
    	mode = 3;
    	strcpy(file_name, file_stem);
    	strcat(file_name, "_32.txt");
    	out_file_lower = fopen(file_name, "wb");				// One file
		out_data_lower = (unsigned char *)malloc(in_size * 9);	// Double words
	...
	if (mode == 0) {
		...
		fwrite(out_data_lower, 1, in_size * 9, out_file_lower);
	    fclose(out_file_lower);
	    free(out_data_lower);
	} else if (mode == 2) {
		// L/U
